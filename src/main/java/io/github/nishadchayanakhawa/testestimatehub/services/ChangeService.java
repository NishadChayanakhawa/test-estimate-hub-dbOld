...
public class ChangeService {
	...
	public ChangeService(ChangeRepository changeRepository, ReleaseService releaseService,
			RequirementRepository requirementRepository, GeneralConfigurationService generalConfigurationService,
	...
	public ChangeDTO save(ChangeDTO changeToSaveDTO) {
		...
		try {
			...
			if (changeToSaveDTO.getId() != null) {
				logger.debug("Before enriching Change: {}",changeToSaveDTO);
				...
				if(changeToSaveDTO.getEstimations()==null) {
					changeToSaveDTO.setEstimations(new HashSet<>());
				}
				logger.debug("After enriching Change: {}",changeToSaveDTO);
			}
	...
	public RequirementDTO saveUseCases(RequirementDTO requirementWithUseCasesToSave) {
		...
		//****************
//		ChangeDTO parentChange=this.get(requirementWithUseCasesToSave.getChangeId());
//		logger.debug("Parent change before enriching: {}", parentChange);
//		Set<RequirementDTO> requirements=parentChange.getRequirements().stream()
//			.map(requirement -> requirement.getId() == originalRequirement.getId()? originalRequirement:requirement)
//			.collect(Collectors.toSet());
//		parentChange.setRequirements(requirements);
//		logger.debug("Parent change after enriching: {}", parentChange);
//		logger.debug("Parent change requirements after enriching: {}", parentChange.getRequirements());
//		this.save(parentChange);
		//***************
		...
		return this.getRequirement(requirementWithUseCasesToSave.getId());
	}
	...
	public ChangeDTO generateEstimates(Long changeId) {
		...
		this.save(modelMapper.map(change, ChangeDTO.class));
